...
import { addDoc, collection, onSnapshot, orderBy, deleteDoc, setDoc, query, doc, serverTimestamp } from "firebase/firestore";
  ...
  return (
    <div className="bg-white my-7 border rounded-sm">
        ...
        <div className="flex items-center p-5">
          <img src={userImg} className="rounded-full w-12 h-12 object-contain border p-1 mr-3" alt="" />
            ...
            <div className="ml-10 h-20 overflow-y-scroll scrollbar-thumb-black scrollbar-thin">
              {
                comments.map((comment) =>(
                  <div key={comment.id} className="flex items-center space-x-2 mb-3">
                    ...
                    <p className="flex-1"> <span className="font-bold ">{comment.data().username}</span> {" "} {comment.data().comment}</p>
